...
@st.cache_resource
def gerar_mapa_folium(df_mapa):
    m = folium.Map(location=[-14.2350, -51.9253], zoom_start=4)
    marker_cluster = MarkerCluster(showCoverageOnHover=False).add_to(m)

    for _, row in df_mapa.iterrows():
        nome = row.get('1. Nome do Ponto ou Pontão de Cultura:', 'Sem Nome')
        folium.Marker(
...
def pagina_visao_geral_dados():
    ...
    with col_mapa:
        # Mapa utiliza apenas coordenadas já presentes na base (base_final.csv)
        df_mapa = filtrado.dropna(subset=['latitude', 'longitude'])
        
        if not df_mapa.empty:
            m = gerar_mapa_folium(df_mapa)
            st_folium(m, width='100%', height=560, returned_objects=[], key='mapa_visao_geral')
        else:
            st.info('Não foi possível localizar pontos com coordenadas válidas.')
    with col_tabela:
